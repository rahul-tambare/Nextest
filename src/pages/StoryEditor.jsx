...
export default function StoryEditor() {
  ...
  const fetchMetrics = async () => {
    try {
      ...
      setModelMetrics(Array.isArray(data) ? data : []);
    } catch (e) {
  ...
  const handleEdit = (story) => {
    ...
    setFormData({
      ...
      ai_model: story.ai_model || null,
      ai_iterations: story.ai_iterations || 0,
    });
    setAiTestCases([]);
    setIsModalOpen(true);
  };
    ...
    try {
      ...
      if (fixed.request_body) {
        setFormData(prev => ({
          ...
          ai_iterations: (prev.ai_iterations || 0) + 1,
        }));
  ...
  const renderStoryDetail = (story) => {
    ...
    return (
      <div className="se-detail-panel animate-slide-up">
        {story.ai_model && (
          <div className="se-detail-row" style={{ borderBottom: '1px solid rgba(129, 140, 248, 0.1)', paddingBottom: 'var(--space-2)', marginBottom: 'var(--space-2)' }}>
            <span className="se-detail-label">✨ AI Origins</span>
            <div className="flex gap-3 items-center">
              <span className="chip" style={{ fontSize: '10px', background: 'rgba(129, 140, 248, 0.1)', color: 'var(--accent-solid)' }}>
                🤖 {story.ai_model}
              </span>
              <span className="chip" style={{ fontSize: '10px' }}>
                🔄 {story.ai_iterations} iterations
              </span>
            </div>
          </div>
        )}
          ...
          <div className="modal se-modal">
            ...
            <div className="se-form-grid">
              ...
              <div className="form-group" style={{ gridColumn: '1 / -1', background: 'rgba(129, 140, 248, 0.05)', padding: 'var(--space-3)', borderRadius: 'var(--radius-md)', border: '1px solid rgba(129, 140, 248, 0.2)' }}>
                ...
                {(() => {
                  const m = modelMetrics.find(x => x.model === aiModel);
                  return (
                    <div className="flex gap-4 items-center" style={{ fontSize: '10px', marginTop: '-4px', marginBottom: 'var(--space-2)', color: 'var(--text-secondary)' }}>
                      {m ? (
                        <>
                          <span title="Average iterations to get a successful payload">📊 Model Track Record: <strong>{Number(m.avg_iterations).toFixed(1)} iterations</strong></span>
                          <span title="Percentage of pass results for stories generated by this model">✅ Success Rate: <strong>{m.success_rate ? Number(m.success_rate).toFixed(0) + '%' : 'N/A'}</strong></span>
                        </>
                      ) : (
                        <span style={{ fontStyle: 'italic' }}>New model selected — no history yet</span>
                      )}
                      {formData.ai_iterations > 0 && (
                        <span style={{ color: '#818cf8', fontWeight: 'bold', marginLeft: 'auto' }}>
                          Iteration: {formData.ai_iterations}
                        </span>
                      )}
                    </div>
                  );
