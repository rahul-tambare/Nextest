...
export default function BugReport() {
  ...
  const [activeTab, setActiveTab] = useState({}); // #14 per-bug active tab
  const [confirmState, setConfirmState] = useState({ open: false, id: null }); // #3

  useEffect(() => { loadBugs(); }, []);

  const loadBugs = async () => {
    try { const data = await api.getBugs(); setBugs(data); } catch {} finally { setLoading(false); }
  };

  const updateStatus = async (id, status) => {
    try { await api.updateBug(id, { status }); toast.success('Bug status updated'); loadBugs(); }
    catch (err) { toast.error(`Failed: ${err.message}`); }
  };

  const updateSeverity = async (id, severity) => {
    try { await api.updateBug(id, { severity }); toast.success('Severity updated'); loadBugs(); }
    catch (err) { toast.error(`Failed: ${err.message}`); }
  };
  ...
  const deleteBug = async () => {
    ...
    try { await api.deleteBug(id); toast.success('Bug deleted'); loadBugs(); }
    catch (err) { toast.error(`Failed: ${err.message}`); }
  };
  ...
  // #14 Load result details for a bug (to show request snapshot)
  const [resultDetails, setResultDetails] = useState({});
  const loadResultDetail = async (resultId) => {
    if (resultDetails[resultId]) return;
    try {
      const results = await api.getResults({ result_id: resultId });
      if (results?.length > 0) {
        setResultDetails(prev => ({ ...prev, [resultId]: results[0] }));
      }
    } catch {}
  };
      ...
      <div className="card" style={{ marginBottom: 'var(--space-6)' }}>
        <div className="card-header" style={{ marginBottom: 0 }}>
          ...
          <div className="flex gap-2">
            <span className="badge badge-neutral" style={{ fontSize: '10px' }}>{bugs.length} total</span>
        ...
        <div className="br-grid">
          {bugs.map((bug) => (
            <div key={bug.id} className="card br-card">
              <div className="br-header">
                ...
                <div className="flex gap-2">
                  <select className="select br-select" value={bug.status} onChange={(e) => updateStatus(bug.id, e.target.value)} style={{ color: statColors[bug.status], borderColor: statColors[bug.status] }}>
                    <option value="open">Open</option><option value="investigating">Investigating</option><option value="fixed">Fixed</option><option value="wontfix">Won't Fix</option>
                  </select>
                  <button className="btn btn-ghost btn-icon" onClick={() => setConfirmState({ open: true, id: bug.id })} style={{ padding: '4px', height: '28px' }}>🗑️</button>
                </div>
              </div>
              ...
              {/* #14 Request/Response Tabs */}
              <div className="br-tabs">
                <button className={`br-tab ${(activeTab[bug.id] || 'response') === 'response' ? 'active' : ''}`} onClick={() => setActiveTab(p => ({ ...p, [bug.id]: 'response' }))}>📥 Response</button>
                <button className={`br-tab ${activeTab[bug.id] === 'analysis' ? 'active' : ''}`} onClick={() => setActiveTab(p => ({ ...p, [bug.id]: 'analysis' }))}>🔍 Analysis</button>
              </div>

              <div className="br-body">
                {(activeTab[bug.id] || 'response') === 'response' && (
                  <>
                    <div className="br-section-title">Response Body</div>
                    <pre className="console" style={{ padding: 'var(--space-2)', margin: 0, fontSize: '11px', maxHeight: '120px' }}>
                      <code>{typeof bug.response_body === 'object' ? JSON.stringify(bug.response_body, null, 2) : bug.response_body}</code>
                    </pre>
                  </>
                )}

                {activeTab[bug.id] === 'analysis' && (
                  <>
